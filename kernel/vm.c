...
void
p_kvmmap(pagetable_t pagetable,uint64 va, uint64 pa, uint64 sz, int perm)
{
...
p_kvminit(void)
{
  ...
  // uart registers
  p_kvmmap(p_kernel_pagetable,UART0, UART0, PGSIZE, PTE_R | PTE_W);

  // virtio mmio disk interface
  p_kvmmap(p_kernel_pagetable,VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);

  // CLINT
  p_kvmmap(p_kernel_pagetable,CLINT, CLINT, 0x10000, PTE_R | PTE_W);

  // PLIC
  p_kvmmap(p_kernel_pagetable,PLIC, PLIC, 0x400000, PTE_R | PTE_W);

  // map kernel text executable and read-only.
  p_kvmmap(p_kernel_pagetable,KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  p_kvmmap(p_kernel_pagetable,(uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  p_kvmmap(p_kernel_pagetable,TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
...
copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len)
{
  ...
  return copyin_new(p->p_kernel_pagetable,dst,srcva,len);
}
...
{
  struct proc *p = myproc();
  return copyinstr_new(p->p_kernel_pagetable,dst,srcva,max);
}
